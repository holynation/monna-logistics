...
function ajaxFormSubmissionSuccess(target,data) {
	try{
		if (typeof(ajaxFormSuccess) === 'function') {
			ajaxFormSuccess(target.attr("name"),data);
      return;
		}
		else{
...
function dateFormatter(dateTime){
  return moment(dateTime, "DD MMM, YYYY");
}

function shortMonth(dt)
{ 
  Date.shortMonths = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'];
  return Date.shortMonths[dt.getUTCMonth()]; 
}

function shortDay(dt){
  Date.shortDays = ["Sun","Mon","Tue","Wed","Thu","Fri","Sat"];
  return Date.shortDays[dt.getUTCDay()];
}

function longDay(dt){
  Date.shortDays = ["Sunday","Monday","Tuesday","Wednesday","Thursday","Friday","Saturday"];
  return Date.shortDays[dt.getUTCDay()];
}

function dayOfWeek(dateTime,type){
  var dt = new Date(dateTime);
  return (type == 'long') ? longDay(dt) : shortDay(dt);
}
...
function timeAgoFormat(dateString) {
  ...
  // const t = Date.parse(endtime) - Date.parse(formatted_date);
  var diff = Math.abs((rightNow.getTime() - then.getTime()) / 1000),
      hours = Math.floor(diff / 3600),
      days = Math.floor(diff / 86400 ),
      weeks = Math.floor(diff / 604800),
      months = Math.floor(diff / 2600640),
      years = Math.floor(diff / 31207680);
      ...
      if(hours <=24){
          ...
          if(days==1){
            ...
            return dayOfWeek(dateString,'long');
          }
      }
      else if(weeks <= 4.3){
          if(weeks==1){
