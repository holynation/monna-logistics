...
// bootstrapping functions
	...
	private function adminInvoices(&$data) {
		...
		$query2 = "SELECT concat(id,'::',amount) as id, name as value from rates";
		$options = buildOptionFromQuery($this->db, $query, [1], isset($_GET['customer']) ? $_GET['customer'] : null);
		$options2 = buildOptionFromQuery($this->db, $query2, null, isset($_GET['rates']) ? $_GET['rates'] : null);
		$data['customerOptions'] = $options;
		$data['rateOptions'] = $options2;
	}
	...
	public function processInvoices() {
		if (!$this->validate([
			...
			'custom_prices' => [
				...
				'rules' => 'required_with[description]',
				'errors' => [
					'required' => 'It appears the rate(s) value is not correctly configured',
				],
			],
			'tax' => [
				'label' => 'tax',
		...
		$pacakageFee = $this->request->getPost('package_fee') ?? 0;
		$certificateFee = $this->request->getPost('certificate_fee') ?? 0;
		...
		for ($i = 0; $i < count($validData['description']); $i++) {
			$subTotal += (float) $validData['weight'][$i] * (float) $validData['custom_prices'][$i];
		}
		...
		for ($i = 0; $i < count($validData['description']); $i++) {
			...
			$temp = explode("::", $validData['rates'][$i]);
			$rates = $temp[0];
			$price = $validData['custom_prices'][$i];
			$quantity = $validData['weight'][$i];
			$customerValue = $validData['custom_value'][$i];
